...
export function ShoppingBagProvider({
  ...
  const addCupcake = useCallback((cupcakeId: number) => {
    const bag = addCupcakeToStorage(cupcakeId);
    setShoppingBag(bag);
  }, []);

  const removeCupcake = useCallback((cupcakeId: number) => {
  ...
  }, [cupcakes, shoppingBag, isLoading, addCupcake, removeCupcake, clearBag]);
